...
Atrinkus šablonus kodo skirstymui į paketus, galima pradėti vertinti jų įtaką sistemai, juos pritaikant esamoms sistemoms.
...
\begin{itemize}
    ...
    Tai matoma sistemos paketų diagramoje\ref{img:crawler_packages_orig}, ciklinėse priklausomybėse dalyvauja 10 paketų ir kartu jie sudaro 5 ciklinių priklausomybių sekas,
...
šios klasės pakete dalinasi vieta dar su 9 klasėmis, todėl yra nepatogu rasti ieškomą įgyvendinimą.
\begin{figure}[H]
    \snugshade
    \dirtree{%
        .1 {/parser} .
...
\end{figure}
Šiam paketui sutvarkyti taikomas \textit{Sąsajų atskyrimo} šablonas - sąsaja iškeliama į atskirą paketą \textit{parse}, jos įgyvendinimai perkeliami į minėto
paketo subpaketus.
...
Naujoje paketų diagramoje galime matyti, jog dabar sistema turi daugiau paketų, tačiau jų priklausomybių kryptys daug aiškesnės:
...
\begin{center}
    ...
    \end{tabular}
\end{center}
Iš gautų rezultatų galime teigti, jog po pertvarkymo šablonai \textit{skirstymas pagal teikiamą funkcionalumą} ir \textit{sąsajų atskyrimas}, išsprendė visas 3 sistemoje matytas problemas,
...
Norint atlikti esybių versijavimą naudojamas \textit{atskirų versijų skirstymo} šablonas, pagal jį kiekvienai esybės versijai sukuriamas atskiras paketas,
taip pat bendras, pasikartojantis kodas iškeliamas į abstrakčią klasę, kuri yra pakete vienu lygiu auksčiau nei versijų paketai:
\begin{figure}[H]
    \snugshade
        \dirtree{%
            .1 {/cryptography} .
...
\end{figure}
Šis pertvarkymas pagal \textit{atskirų versijų skirstymo} šabloną, atskiria esybės versijas ir pristato kelis paketus, apibrėžiančius aiškesnius funkcionalumus.
...
yra subalansuoti, tačiau šis skirstymas turėjo ir neigiamų padarinių - sukelė 5 ciklines priklausomybes.
Šiais priklausomybes galima matyti sistemos paketų diagramoje:
\begin{figure}[H]
    \centering
...
\end{figure}
Norint panaikinti ciklines priklausomybes, reikia panaudoti šabloną \textit{skirstymas pagal teikiamą funkcionalumą} ir, išskaidant pagrindinį paketą
...
Taip pat vadovaujantis \textit{pagalbinių klasių priskyrimas dalykinės srities paketui} šablonu, visos pagalbinės klasės, neturinčios priklausomybių į kitus paketus ir
...
vis dar yra gan mažas, ciklinės priklausomybės išnyko, todėl galime teigti, kad \textit{skirstymo pagal teikiamą funkcionalumą},
