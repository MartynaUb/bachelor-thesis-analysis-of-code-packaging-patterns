 ...
 */
class ServerBasedCrawlingPermissions implements CrawlingPermissions {

    private static final Logger logger = LoggerFactory.getLogger(ServerBasedCrawlingPermissions.class);
    ...
    ServerBasedCrawlingPermissions(RobotstxtConfig config, PageFetcher pageFetcher) {
        this(config, pageFetcher, 16384);
    }

    private ServerBasedCrawlingPermissions(RobotstxtConfig config, PageFetcher pageFetcher, int maxBytes) {
        this.config = config;
    ...
    private HostDirectives fetchDirectives(URL url) throws IOException, InterruptedException {
        ...
        try {
            ...
            if (fetchResult.getStatusCode() == HttpStatus.SC_OK) {
                ParsedPage page = Page.newPage(robotsTxtUrl);
